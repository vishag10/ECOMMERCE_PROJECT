...
export default function Orderstatus() {
  ...
  const getOrderProgress = (index) => {
    // Mock status for demonstration
    const statuses = [
      { id: 1, title: "Order Placed", completed: true, date: new Date(Date.now() - 3*24*60*60*1000).toLocaleDateString() },
      { id: 2, title: "Packed", completed: index % 2 === 0, date: new Date(Date.now() - 2*24*60*60*1000).toLocaleDateString() },
      { id: 3, title: "Shipped", completed: false, date: new Date(Date.now() - 1*24*60*60*1000).toLocaleDateString() },
      { id: 4, title: "Delivered", completed: false, date: calculateDeliveryDate() }
    ];
    return statuses;
  };
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <button className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-medium py-2 px-6 rounded-md shadow-md hover:shadow-lg transition duration-300">
              Continue Shopping
            </button>
          </div>
        ) : (
                ...
                <div className="p-5 border-b">
                  <div className="mb-4 flex justify-between items-center">
                    ...
                    <span className="text-sm bg-green-100 text-green-700 py-1 px-3 rounded-full font-medium">
                      Estimated: {calculateDeliveryDate()}
                    </span>
                  </div>
                  ...
                  <div className="relative">
                    <div className="overflow-hidden mt-2">
                      <div className="flex">
                        {getOrderProgress(index).map((step, idx) => (
                          <div key={step.id} className="flex-1 relative">
                            <div className="flex items-center">
                              <div className={`h-10 w-10 rounded-full border-2 flex items-center justify-center z-10 relative ${
                                step.completed ? 'bg-green-500 border-green-500 text-white' : 'bg-white border-gray-300 text-gray-500'
                              } shadow-md`}>
                                {step.completed ? (
                              ...
                              {idx < getOrderProgress(index).length - 1 && (
